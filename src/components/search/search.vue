  ...
  <div class="search">
    ...
    <div class="shortcut-wrapper" v-show="!query" ref="shortcutWrapper">
      <scroll class="shortcut" :data="shortcut" ref="shortcut" :refreshDelay="refreshDelay">
        <!--需要把热门搜索和搜索历史用一个标签包裹起来，然后把两者的数据连接起来，传给scroll组件，其中一个发送变化时都会执行refresh-->
        <div>
          <!--热门搜索-->
          <div class="hot-key">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    watch: {
      /**
       * 监听query变化，
       * 在搜索时，会切换到显示搜索结果的界面，此时的话query如果有变化【从有到无】，是不会和热门搜索以及搜索历史有关联的，
       * 这时如果再切换到热门搜索的界面就不会滚动了，需要手动刷新scroll组件
       * @param newQuery
       */
      query(newQuery) {
        if (!newQuery) {
