          ...
          <div class="middle-l" ref="middleL">
            ...
            <div class="cd-wrapper" ref="cdWrapper">
              <div class="cd" ref="imageWrapper">
                <img class="image" :class="cdCls" :src="currentSong.image" ref="image">
              </div>
            </div>
            <!--当前歌词-->
  ...
  export default {
    ...
    methods: {
      ...
      enter(el, done) {
        ...
        // 使用第三方库，运行动画，并传入回调函数done，跳转到afterEnter
      ...
      /**
       * 监听监听audio的ended事件方法
       * 重置播放时间为0，如果是循环模式则进入调用循环播放方法，否则，播放下一首
       */
      end() {
        this.currentTime = 0
      ...
      /**
       * 监听audio的pause事件，设置播放状态为false,并且停止歌词
       */
      paused() {
      ...
      /**
       * 监听进度条子组件派发的进度条touchEnd和click事件
      ...
      /**
       * 监听中间部分touchMove事件，操作CD页面和歌词页面的实时滑动
       * @param e
       */
      middleTouchMove(e) {
        ...
        // 定义一个左偏距 当页面为左边的CD页面时为0， 为歌词页面时为屏幕宽度（负值）【从右滑动到左为负值】
      ...
      /**
       * 监听中间部分touchEnd事件，操作CD页面和歌词页面的最终滑动状态
       */
      middleTouchEnd() {
        if (!this.touch.moved) {
          return
    ...
    watch: {
      ...
      currentSong(newSong, oldSong) {
        ...
        clearTimeout(this.timer)
        // 歌曲变化后，定时5秒，把songReady状态设置为true
      ...
      playing(newPlaying) {
        ...
        // newPlaying为true播放歌曲，为false暂停播放
        this.$nextTick(() => {
